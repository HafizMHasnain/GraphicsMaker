...
//    }
    ...
    public static String getRealPathFromURI(Uri contentURI, Context context) {
        try {
            ...
            if ("file".equalsIgnoreCase(contentURI.getScheme())) {
                return contentURI.getPath(); // Return file path directly for file scheme
            } else {
                return null; // Could not retrieve path
            }
    ...
    public static Bitmap resizeBitmap(Bitmap bit, int width, int height) {
        ...
        float originalWidth = (float) bit.getWidth();
        float originalHeight = (float) bit.getHeight();
        float targetWidth = (float) width;
        float targetHeight = (float) height;

        // Determine if aspect ratio adjustment is needed
        boolean maintainAspectRatio = false; // Set to true if you want to preserve aspect ratio
        ...
        if (maintainAspectRatio) {
            ...
            if (originalAspect > targetAspect) {
                // Wider than target: scale width to fit, adjust height
                targetHeight = targetWidth / originalAspect;
            } else {
                // Taller than target: scale height to fit, adjust width
                targetWidth = targetHeight * originalAspect;
            }
        }

        // Create scaled bitmap with target dimensions
        return Bitmap.createScaledBitmap(bit, (int) targetWidth, (int) targetHeight, true);
    }
...
//        float rat1 = wd / he;
//        float rat2 = he / wd;
////        Log.d("CreatePoster","resizeBitmap init");
////        Log.d("CreatePoster",String.valueOf(wr));
////        Log.d("CreatePoster",String.valueOf(hr));
////        Log.d("CreatePoster",String.valueOf(wd));
////        Log.d("CreatePoster",String.valueOf(he));
////        Log.d("CreatePoster",String.valueOf(rat1));
////        Log.d("CreatePoster",String.valueOf(rat2));
//        if (wd > wr) {
////            Log.d("CreatePoster","wd > wr");
//            if (wr * rat2 > hr) {
////                Log.d("CreatePoster","wr * rat2 > hr");
//                he = hr;
//                wd = he * rat1;
//            } else {
//                wd = wr;
//                he = wd * rat2;
////                Log.d("CreatePoster","wr * rat2 > hr else");
//            }
//        } else if (he > hr) {
////            Log.d("CreatePoster","he > hr");
//            he = hr;
//            wd = he * rat1;
//            if (wd > wr) {
////                Log.d("CreatePoster","wd > wr");
...
//    }
    ...
    public static Bitmap mergelogo(Bitmap bitmap, Bitmap logo, float screenSize, float largeSize) {
        ...
        if (wd > wr) {
            ...
            logo = Bitmap.createScaledBitmap(logo, (int) ((wd * largeSize) / screenSize), (int) ((he * largeSize) / screenSize), false);
        }
    ...
    public static Bitmap getTiledBitmap(Context ctx, int resId, int width, int height, boolean flag) {
        ...
        paint.setShader(new BitmapShader(t, TileMode.REPEAT, TileMode.REPEAT));
        Bitmap b = Bitmap.createBitmap(width, height, Config.ARGB_8888);
        new Canvas(b).drawRect(rect, paint);
        return b;
    }

    public static Bitmap getTiledBitmap(Activity activity, String resId, SeekBar seek_tailys, int width, int height) {
        Rect rect = new Rect(0, 0, width, height);
        Paint paint = new Paint();
        int prog = seek_tailys.getProgress() + 10;
        Options options = new Options();
        options.inScaled = false;
        byte[] bytes = JniUtils.decryptResourceJNI((Context) activity, resId);
        paint.setShader(new BitmapShader(Bitmap.createScaledBitmap(BitmapFactory.decodeByteArray(bytes, 0, bytes.length, options), prog, prog, true), TileMode.REPEAT, TileMode.REPEAT));
        Bitmap b = Bitmap.createBitmap(width, height, Config.ARGB_8888);
        new Canvas(b).drawRect(rect, paint);
        return b;
    }

    public static Bitmap getBitmapOfPosterCard(Activity activity, String resId) {
        // Options to decode the bitmap without scaling
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inScaled = false; // Prevent any scaling

        // Decrypt the resource to get the raw byte array
        byte[] bytes = JniUtils.decryptResourceJNI(activity, resId);

        // Decode the byte array into a bitmap
        Bitmap sourceBitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length, options);

        if (sourceBitmap == null) {
            throw new IllegalArgumentException("Failed to decode bitmap from resource: " + resId);
        }

        // Return the decoded bitmap, which is of the actual size
        return sourceBitmap;
    }
