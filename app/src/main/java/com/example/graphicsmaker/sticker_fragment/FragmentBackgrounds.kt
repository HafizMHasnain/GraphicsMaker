...
class FragmentBackgrounds() : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_my_backgund, container, false)
        ...
        this.getImage = requireActivity() as OnGetImageOnTouch?
        this.gridView = view.findViewById<View>(R.id.gridview) as GridView
        gridView!!.adapter =
            FrameAdapter(requireActivity(), Constants.backgrounds_list, this.remove_ad_pref!!, this.prefs!!, lock,FragmentBackgrounds::class.java.name)
        gridView!!.onItemClickListener = object : AdapterView.OnItemClickListener {
            override fun onItemClick(
                ...
                if (lock.contains(position)) {
                    this@FragmentBackgrounds.showPremiumDialog(position)

                } else if (remove_ad_pref!!.getBoolean("isAdsDisabled", false)) {
                    getImage!!.ongetPosition(
                        Constants.backgrounds_list[position],
                        "Background",
                        "",
                        null,
                        null,
    ...
    private fun showPremiumDialog(position: Int) {
        ...
        dialog.findViewById<View>(R.id.no_thanks).setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                this@FragmentBackgrounds.remove_watermark_video_dialog(position)
                dialog.dismiss()
            }
        })
        lp.dimAmount = 0.7f
    ...
    fun remove_watermark_video_dialog(position: Int) {
        ...
        remove_watermark_msg.text = requireActivity().resources.getString(R.string.use_background)
        remove_watermark_msg.typeface = ttf
        val no_thanks = dialog.findViewById<View>(R.id.no_thanks) as Button
        no_thanks.typeface = ttf
        no_thanks.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                dialog.dismiss()
            }
        })
        ...
        watch_ad.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                dialog.dismiss()
                getImage!!.ongetPosition(
                    Constants.backgrounds_list[position],
    ...
    override fun onDestroyView() {
        ...
        try {
            Thread(object : Runnable {
                override fun run() {
                    try {
                        Glide.get((this@FragmentBackgrounds.requireActivity())).clearDiskCache()
                        Thread.sleep(100)
