...
class FragmentTexture() : Fragment() {
    ...
    override fun onCreateView(
        ...
        gridView!!.adapter =
            FrameAdapter(requireActivity(), Constants.textures_list, this.remove_ad_pref!!, this.prefs!!,lock,FragmentTexture::class.java.name)
        gridView!!.onItemClickListener = object : AdapterView.OnItemClickListener {
            override fun onItemClick(
                ...
                if (lock.contains(position)) {
                    this@FragmentTexture.showPremiumDialog(position)
                }
                else if (remove_ad_pref!!.getBoolean("isAdsDisabled", false)) {
                    getImage!!.ongetPosition(
                        Constants.textures_list[position],
                        "Texture",
                        "",
                        null,
                        null,
    ...
    fun showPremiumDialog(position: Int) {
        ...
        dialog.findViewById<View>(R.id.no_thanks).setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                this@FragmentTexture.remove_watermark_video_dialog(position)
                dialog.dismiss()
            }
        })
        lp.dimAmount = 0.7f
    ...
    override fun onDestroyView() {
        ...
        try {
            Thread {
                try {
                    Glide.get((this@FragmentTexture.requireActivity())).clearDiskCache()
                    Thread.sleep(100)
